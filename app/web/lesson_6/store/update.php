...
<div class="content">
    ...
    try {
        ...
        $query = $dbh->query("SELECT `headline`, `thumb_adress`, `description`, `price` FROM `goods` INNER JOIN  `pictures` ON `goods`.`id` = `pictures`.`goods_id` WHERE `goods`.`id` = $id")->fetch(PDO::FETCH_ASSOC);
    ...
    <div class="load">
        <form enctype="multipart/form-data" method="post" action="models/update.php">
            ...
            <p><img src="<?=$query['thumb_adress']?>" alt=""></p>
